...
log_file=$(printf "marc_log/%s_%s_%s_%s.csv" $replication $blocksize $dimension $completedmaps)
...
hadoop \
	...
	$dir \
	$result_dir | tee $(printf "/root/marc_log/terasort/%s_%s_%s_%s.txt" $replication $blocksize $dimension $completedmaps)
